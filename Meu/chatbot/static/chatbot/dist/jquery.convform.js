...
ConvState.prototype.printQuestion = function () {
    ...
    setTimeout(function () {
        ...
        if (this.current.input.hasOwnProperty('noAnswer') && this.current.input.noAnswer === true) {
            if (this.next()) {
                setTimeout(function () {
                    var messageObj = $('<div class="message to ready" style="background-color:transparent; position:static;"><img src="/static/chatbot/img/head.png" style="width:50px; height:50px; position:absolute; margin-left: -15px; margin-top: -2%;"></div><div class="message to typing" style="margin-left: 55px; margin-rigth:0%"><div class="typing_loader"></div></div>');
...
ConvState.prototype.printAnswers = function (answers, multiple) {
    ...
    if (multiple) {
        ...
        for (var i in answers) {
            var tmp = answers
            tmp[i].text = "<" + answers[i].text +">"
            // console.log(tmp[i].text)
            if (answers.hasOwnProperty(i)) {
                var option = $('<div class="option">' + tmp[i].text + '</div>')
                    .data("answer", tmp[i])
                    .click(function (event) {
                        this.current.input.selected = $(event.currentTarget).data("answer").value;
                        console.log(this.current.input.selected)
                        // console.log(answerObject)
                        $.ajax({
                            url: '',
                            type: 'GET',
                            data: {
                                "question_ch": this.current.input.selected
                            },
                        ...
                        this.wrapper.find(this.parameters.inputIdHashTagName).removeClass('error');
                        this.wrapper.find(this.parameters.inputIdHashTagName).val('');
                        this.answerWith($(event.currentTarget).data("answer").value, $(event.currentTarget).data("answer"));
                        this.wrapper.find('div.options div.option').remove();
                    }.bind(this));
                this.wrapper.find('div.options').append(option);
                $(window).trigger('dragreset');
...
ConvState.prototype.answerWith = function (answerText, answerObject) {
    // console.log('previous answerText: ', answerText);
    // console.log('previous answerObject: ')

    if (this.current.input.hasOwnProperty('talk')) {
        $.ajax({
            url: 'conversation/',
            type: 'GET',
            data: {
            ...
            success: function (res) {
                // console.log('After view_1: ', res)
                // console.log(answerObject)
                // console.log(answerText)
                // html = '<div class="message to ready" style="background-color:transparent;'
                // html += 'position:static;"><img src="/static/chatbot/img/head.png" style="width:50px;'
                // html += 'height:50px; position:absolute; margin-left: -15px; margin-top: -2%;"></div>'
                html = '<div class="message to ready" style="margin-left: 55px; margin-rigth:0%">' + res + '</div>'
                
                $('.message.to.typing').remove();
        ...
        */
        ...
        if (inputs.length) {
            ...
            switch (parameters.typeInputUi) {
                case 'input':
                    inputForm = $('<form id="' + parameters.formIdName + '" class="convFormDynamic"><div class="options dragscroll"></div><input id="' + parameters.inputIdName + '" type="text" placeholder="' + parameters.placeHolder + '" class="userInputDynamic"></><button type="button" class="submit" >' + parameters.buttonText + '</button><span class="clear"></span></form>');//
                    break;
                case 'textarea':
                    inputForm = $('<form id="' + parameters.formIdName + '" class="convFormDynamic"><div class="options dragscroll"></div><textarea id="' + parameters.inputIdName + '" rows="1" placeholder="' + parameters.placeHolder + '" class="userInputDynamic"></textarea><button type="button" class="submit">' + parameters.buttonText + '</button><span class="clear"></span></form>');//
                    break;
                default:
